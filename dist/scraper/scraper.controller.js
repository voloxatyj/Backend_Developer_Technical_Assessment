"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ScraperController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const scraper_request_dto_1 = require("./dto/scraper-request.dto");
const scraper_service_1 = require("./scraper.service");
const scraper_response_dto_1 = require("./dto/scraper-response.dto");
let ScraperController = class ScraperController {
    constructor(scraperService) {
        this.scraperService = scraperService;
    }
    scrapeRequest({ url, d }) {
        return this.scraperService.scrape({ url, d });
    }
};
exports.ScraperController = ScraperController;
__decorate([
    (0, swagger_1.ApiOperation)({
        summary: "Initiate a new scraping process for the provided URL",
    }),
    (0, swagger_1.ApiOkResponse)({ type: scraper_response_dto_1.ScraperResponseDto, status: 200 }),
    (0, swagger_1.ApiNotFoundResponse)({ status: 404, description: "Resource not found" }),
    (0, swagger_1.ApiBadRequestResponse)({ status: 400, description: "Bad Request" }),
    (0, common_1.Get)("scrape"),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [scraper_request_dto_1.ScraperRequestDto]),
    __metadata("design:returntype", Promise)
], ScraperController.prototype, "scrapeRequest", null);
exports.ScraperController = ScraperController = __decorate([
    (0, common_1.Controller)("scraper"),
    (0, swagger_1.ApiTags)("scraper"),
    __metadata("design:paramtypes", [scraper_service_1.ScraperService])
], ScraperController);
//# sourceMappingURL=scraper.controller.js.map